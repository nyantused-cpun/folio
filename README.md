...
记忆可信度设计理念（证据溯源 / 三态表达 / 冲突保留 / 检查点）受 [openTrinity/mycontext](https://github.com/openTrinity/mycontext)（Elastic License 2.0）启发；Folio 的实现为独立原创（标识符、数据结构、架构均无复制，零复制验证在内部维护）。
