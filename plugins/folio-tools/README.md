...
- `ctx.tools.register` / `defineTool`：dsh-tools 官方注册面（rc.8 逐行核实）；注册层 = 调用方 ctx scope（profile 层 = host 全局）；
...
- 沙箱内：`node --check index.js defs.js guard.js` + `node test/run-check.mjs`（2026-08-14：10/10 通过；2026-08-16 增补 guard 纯函数断言；2026-08-20：rc.8 源码复核 + import 冒烟通过）；
